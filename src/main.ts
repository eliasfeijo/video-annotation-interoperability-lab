...
const renderer = (params.get("renderer") === "b" && exp !== "7-animate" ? "b" : "a") as RendererKind;
...
if (!ELEMENT || !viewport) throw new Error("missing stage-root/viewport elements");

const stage = new Stage(ELEMENT);
stage.setSanitizer(sanitizeOn ? allowlistSanitizer : identitySanitizer);
stage.setFit(fit);
...
async function boot(): Promise<void> {
  const manifestUrl = `${location.origin}/manifests/${MANIFEST_MAP[exp] ?? `exp${exp}.json`}`;
  ...
  if (exp === "7" && expRef?.keyframesUrl) {
    ...
    const byId = new Map<string, any[]>(
      (ext.annotations ?? []).map((a: any) => [a.id, (a.keyframes as any[]) ?? []]),
    );
    for (const ov of r.overlays) {
      const keys = byId.get(ov.id);
      if (keys?.length) ov.keyframes = keys.map((k) => ({ t: k.t, x: k.x, y: k.y }));
    }
  }
  ...
  stage.setCanvas(canvasInfo);
  stage.setOverlays(renderer === "b" ? resolvedB : resolvedA);
  if (videoUrl) {
...
void boot().then(() => {
  ...
  window.__lab = {
    ...
    parity: () => {
      const d = resolvedA.map((a, i) =>
        sameOverlay(a, resolvedB[i] ?? ({ id: "(missing)" } as unknown as ResolvedOverlay)),
      );
    ...
    overlayRect: (id) => {
      const g = stage.overlaySvg.querySelector(`[data-overlay-id="${id}"] [data-overlay-nested]`);
    ...
    layerCount: () => resolvedA.length,
    setFit: (f) => stage.setFit(f),
    setSanitize: (on) => {
      stage.setSanitizer(on ? allowlistSanitizer : identitySanitizer);
      stage.setOverlays(renderer === "b" ? resolvedB : resolvedA);
    },
    domProbe: (id, selector) => {
      const g = stage.overlaySvg.querySelector(`[data-overlay-id="${id}"]`);
      const el = g?.querySelector(selector);
      return el ? el.getAttribute("transform") ?? el.getAttribute("x") ?? el.getAttribute("cx") ?? null : null;
    },
  };
  ...
  document.dispatchEvent(new CustomEvent("lab-ready", { detail: { exp, renderer } }));
});
